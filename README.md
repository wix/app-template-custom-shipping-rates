...
#### Important notes
- The end to end actions relies on the site name, please update the site name in the ci [configuration file](./ci.config.json) for the relevant platform.
- Some deployment platforms do not copy workflows to the cloned repo, if you do not see any files under `.github/workflows` please copy the relevant file from the [template repo](https://github.com/wix/app-template-custom-shipping-rates) to your repo.
...
To update the screenshots simply execute the workflow manually, and set the value of the workflow input `shouldUpdateBaseline` to `true`.<br>
Don't forget to run it on the relevant deployment platform/s.<br>
